...
export const  generateQuestion = async (req, res) => {
  try {
    ...
    if (!userPrompt.trim()) {
        ...
        return res.status(400).json({
            message: "Prompt content is emopty."
        });
...
`
    ...
    const interview = await Interview.create({
        ...
        question: questionArray.map((q, index)=>({
            question: q,
...
export const finishInterview = async (req,res) => {
    try {
        ...
        return res.status(500).json({message: `failed to finish ${error} Interview`})
...
export const getInterviewReport = async (req,res) => {
    try {
        ...
        return res.status(500).json({message:`failed to find cuurentUser Interview report ${error}`}) 
    }
}